...
export default function HeroCalculator() {
  ...
  const selectClass = "w-20 sm:w-[105px] shrink-0 rounded-[var(--radius-sm)] border border-outline-variant bg-surface-container-low px-2 sm:px-2.5 py-2 text-[13px] font-medium text-on-surface outline-none focus:border-primary transition-colors";

  return (
    <div className="rounded-[var(--radius-xl)] bg-surface-container-lowest p-4 sm:p-5 w-full max-w-[340px] sm:max-w-sm" style={{ boxShadow: "var(--shadow-level5)" }}>
      <div>
        <label className="block text-[11px] font-medium uppercase tracking-widest text-outline mb-1.5">You send</label>
        <div className="flex gap-2">
          <select value={from} onChange={(e) => { setFrom(e.target.value); if (e.target.value === to) setTo(from); }} className={selectClass}>
            {currencies.map((c) => <option key={c.code} value={c.code}>{c.flag} {c.code}</option>)}
          </select>
          <input type="text" inputMode="decimal" value={amount}
            onChange={(e) => setAmount(e.target.value.replace(/[^0-9.]/g, ""))}
            className="flex-1 min-w-0 rounded-[var(--radius-sm)] border border-outline-variant px-3 py-2 text-[15px] sm:text-[16px] font-semibold text-on-surface tabular-nums outline-none focus:border-primary transition-colors text-right" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <select value={to} onChange={(e) => { setTo(e.target.value); if (e.target.value === from) setFrom(to); }} className={selectClass}>
            {currencies.map((c) => <option key={c.code} value={c.code}>{c.flag} {c.code}</option>)}
          </select>
          <div className="flex-1 min-w-0 rounded-[var(--radius-sm)] border border-outline-variant bg-surface-container-low px-3 py-2 text-[15px] sm:text-[16px] font-semibold text-on-surface tabular-nums text-right">
      ...
      {rate && from !== to && (
        <p className="mt-3 text-[10px] sm:text-[11px] text-outline text-center">
          Indicative: 1 {from} = {rate.toLocaleString(undefined, { minimumFractionDigits: 4, maximumFractionDigits: 4 })} {to}
        </p>
      )}

      <Link href="/quote" className="mt-3 sm:mt-4 flex items-center justify-center gap-1.5 w-full rounded-[var(--radius-xl)] bg-primary px-4 py-2.5 text-[14px] font-medium text-on-primary hover:opacity-90 transition-opacity">
        Get live rate <ArrowRight className="h-3.5 w-3.5" />
      </Link>

      <p className="mt-2 text-[10px] text-outline text-center leading-snug">
